...
<script type="text/javascript">
    ...
    function page(e, last) {
        ...
        var path = $(e.target).parent().parent('ul').data('path');
        if (page === "Prev") {
            if (index >= 1) {
                index--;
                page = index;
        ...
        if (path == baseUrl + "/search/getlist") {
            ...
            var url = path + "?entry=" + entry + "&page=" + page + "&search=" + search;
    ...
    function logout(e) {
        e.preventDefault();
        var xhr = ajaxGetObj('<?php echo url('/') . '/admin/auth/logout'; ?>');
        xhr.done(function (resp) {
