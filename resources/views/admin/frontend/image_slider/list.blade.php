...
<div class="box col-md-6">
                    ...
                    <h3 class="box-title">List of Image Slider <a class="btn btn-primary" href="{{url('/admin').'/image-slider/create'}}"><i class="fa fa-plus"></i> Add New Slider</a></h3>
...
function createTable(resp){
    createDataTableBrand('material-table',resp,['title','banner_image'],'id',0,1);
}
    ...
    if ($('#' + domId).length) {
        ...
        if (data.length) {
            ...
            for (var i in data) {
                ...
                if (actions != 1) {
                    row.insertCell(fields.length).innerHTML = "<a href='"+baseUrl+"/"+data[i][pk]+"/edit' class='btn btn-xs btn-primary' title='Edit'><i class='glyphicon glyphicon-edit'></i></a> &nbsp;&nbsp; <a href='javascript:void(0)' onclick=\"delt('" + data[i][pk] + "')\" class='btn btn-xs btn-danger' title='Delete'><i class='glyphicon glyphicon-trash'></i></a>";
                }
                if (sn == 1) {
