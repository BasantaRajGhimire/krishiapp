    ...
    <div id="second-content-section" class="col-md-12">
        <div class="box col-md-6">
            <div class="box-header with-border">
                <h3 class="box-title">List of Contact Form</h3>
                ...
                <div id="showtable" class="box-body table-responsive">
                    <table id="material-table" class="table table-striped table-bordered table-hover">
                     <tr id="heading">
                        ...
                        <th id="date" style="width:15%">Created Date</th>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Send Message to Email: <span id="subject-user"></span></h4>
            </div>
            <div class="modal-body">
                <form id="bid-form" role="form" action="{{url('/admin/contactus/send-mail')}}" onsubmit="sendMessage(event)" method="post" id="form" class="">
                    <input type="hidden" value="" name="id" id="id">
                    <input type="hidden" value="" name="status" id="status1">
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                ...
                                <div id="message-err" class="error"></div>
                            </div>
                        </div>
                    </div>
...
<script>
var baseUrl = "<?php echo url('admin/contactus'); ?>";
function table(lp=null){
    var status = $('#status').val();
    var stat = status || 'PENDING';
    if(stat == 'APPROVED'){
        $('#actions').hide();
        $('#heading').append('<td id="replied">Replied Message</td>');
...
function createTable(resp){
    createDataTablContactRequest('material-table',resp,['name','email','message','created_at'],'id',0,1);
}
function createDataTablContactRequest(domId, response, fields, pk, actions, sn) {
    if ($('#' + domId).length) {
        ...
        if (data.length) {
            ...
            for (var i in data) {
                ...
                if (actions != 1) {
                    if(data[i]['status'] == 'PENDING'){
                        row.insertCell(fields.length).innerHTML = "<a href='javascript:void(0)' onclick=\"ShowMessageModal('" + data[i][pk] + "','1')\" class='btn btn-xs m-1 btn-success' title='Approve'>Complete Task</a><a href='javascript:void(0)' onclick=\"ShowMessageModal('" + data[i][pk] + "','2')\" class='btn btn-xs m-1 btn-danger' title='Reject'>Reject Task</a>";
                    }
            ...
            createPaginationContact(response);
        } else {
...
function createPaginationContact(resp) {
...
function ShowMessageModal(id,s){
    // console.log(s);
    $('#status1').val(s);
    $('#id').val(id);
