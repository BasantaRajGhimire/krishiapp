...
<div id="ticket-modal" class="modal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title"><i class="fa fa-ticket"></i> Ticket Details
...
<script>
    var baseUrl = "<?php echo url('admin/client-ticket/open'); ?>";
    ...
    function createTable(resp) {
        createOpenTicketTable('material-table', resp, ['ticket_id', 'client_name', 'name', 'title','message','screenshot','created_at'], 'id',0,1);
    }

    function createOpenTicketTable(domId, response, fields, pk, actions, sn) {
        var pathUrl =  "{{url('client-ticket')}}";
        if ($('#' + domId).length) {
            ...
            if (data.length) {
                ...
                for (var i in data) {
                    ...
                    if (actions != 1) {
                        row.insertCell(fields.length).innerHTML = "<a href='javascript:void(0)' data-id =" + data[i][pk] + " class='btn btn-xs btn-primary btn-process' >Process</a>&nbsp;&nbsp;<a href='javascript:void(0)' data-id =" + data[i][pk] + " class='btn btn-xs btn-danger btn-complete' title='Complete'>Complete</a>";
                    }
                    if (sn == 1) {
    ...
    function process(id) {
        var url = id + '?status=Processing&remarks=' + $('#ticket-modal .remarks').val();
        var xhr = ajaxGetObj(url);
        xhr.done(function (resp) {
            ...
            if (reason.responseJSON.errors.remarks) {
                $('#ticket-modal .remarks').focus();
                $('#ticket-modal .remarks').after('<span class="error-msg" style="color: red;">*' + reason.responseJSON.errors.remarks + '</span>');
            } else {
    ...
    $(document).on('click', ".btn-process", function (e) {
        $('.error-msg').remove();
        var id = $(this).data('id');
        $('#ticket-modal').data('id', id).modal('show');
    });

    $('#ticket-modal .btn-submit').click(function () {
        // handle deletion here
        var id = $('#ticket-modal').data('id');

        process(id);

    });

    $('#ticket-modal').on('hidden.bs.modal', function (event) {
        $('#ticket-modal .remarks').val('');
    });
