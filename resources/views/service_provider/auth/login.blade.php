        ...
        <div class="row align-items-center">
          <div class="col-12 mt-5 mb-5 pt-3 pb-3 text-center">
            ...
            <h3 class="text-warning">
              Sign In and Browse for Business Oppurtunities.
            </h3>
          </div>
        </div>
      </div>
      ...
      <div class="container d-block m-auto">
        <div class="row">
          <div class="col-9 col-sm-8 col-md-5 mx-auto">
            <div class="card">
              <div class="card-body">
                <div class="error {{ Session::has('err')?'show':''}}">
                  @if(Session::has('err'))
                  {{ Session::get('err') }}
                  @endif
                </div>
                ...
                <div class="text-center">
                  <hr>
                  <a href="{{url('service-provider/auth/forget-password')}}">Forgot Password?</a><br />
                  <span>Or Sign up
                    <a href="{{url('service-provider/register')}}">
                      <b>
                        here
                      </b>
                    </a>
...
function login(e) {
    ...
    if ($('#email').val() == '' || $('#password').val() == '') {
        ...
        var url = "<?php echo url('/') . '/service-provider/auth/login'; ?>"
        var data = $('#loginForm').serialize();
        var xhr = submitFormAjax(url, data);
        xhr.done(function (resp) {
            //toast(resp);
            window.location.href = '<?php echo url('/service-provider'); ?>';
...
function setTitle() {
    ...
    if (title) {
        ...
        $('.body-header').html('');
        $('title').text('ServiceProvider | Log In');
    }
}
setTitle();
</script>